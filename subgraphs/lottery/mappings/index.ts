...
export function handleLotteryOpen(event: LotteryOpen): void {
  ...
  lottery.ticketPrice = toBigDecimal(event.params.priceTicketInCham);
...
export function handleTicketsClaim(event: TicketsClaim): void {
  ...
  if (user !== null) {
    user.totalCham = user.totalCham.plus(toBigDecimal(event.params.amount));
    user.save();
  }
