...
export function handleAskNew(event: AskNew): void {
  ...
  if (token === null) {
    ...
    if (
      event.params.collection.equals(
        Address.fromString(PANCAKE_BUNNIES_ADDRESS)
      )
    ) {
      token.otherId = fetchBunnyId(
        event.params.collection,
        event.params.tokenId
      );
    }
    token.collection = collection.id;
    token.metadataUrl = fetchTokenURI(
      event.params.collection,
      event.params.tokenId
    );
...
export function handleTrade(event: Trade): void {
  ...
  if (buyer === null) {
    ...
    buyer.totalVolumeInBNBTokensPurchased = toBigDecimal(
      event.params.askPrice,
      18
    );
    buyer.totalVolumeInBNBTokensSold = ZERO_BD;
    buyer.totalFeesCollectedInBNB = ZERO_BD;
    buyer.averageTokenPriceInBNBPurchased =
      buyer.totalVolumeInBNBTokensPurchased;
    buyer.averageTokenPriceInBNBSold = ZERO_BD;
  } else {
    buyer.numberTokensPurchased = buyer.numberTokensPurchased.plus(ONE_BI);
    buyer.totalVolumeInBNBTokensPurchased =
      buyer.totalVolumeInBNBTokensPurchased.plus(
        toBigDecimal(event.params.askPrice, 18)
      );

    buyer.averageTokenPriceInBNBPurchased =
      buyer.totalVolumeInBNBTokensPurchased.div(
  ...
  seller.averageTokenPriceInBNBSold = seller.totalVolumeInBNBTokensSold.div(
    seller.numberTokensSold.toBigDecimal()
  );
  ...
  if (collection !== null) {
    ...
    collection.totalVolumeBNB = collection.totalVolumeBNB.plus(
      toBigDecimal(event.params.askPrice, 18)
    );
    collection.numberTokensListed = collection.numberTokensListed.minus(ONE_BI);
    collection.save();
  ...
  token.tradeVolumeBNB = token.tradeVolumeBNB.plus(
    token.latestTradedPriceInBNB
  );
  token.updatedAt = event.block.timestamp;
...
export function handleRevenueClaim(event: RevenueClaim): void {
  ...
  user.totalFeesCollectedInBNB = user.totalFeesCollectedInBNB.plus(
    toBigDecimal(event.params.amount, 18)
  );
  user.save();
}
